...
class _ARViewState extends ConsumerState<ARView> {
  ...
  Widget build(BuildContext context) {
    final clients = ref.watch(arFilteredClientsProvider); // List<ClientAR>
    ...
    final pagePadding = EdgeInsets.all(isTablet ? 24 : 20);
    final headerTitleStyle = TextStyle(
      fontSize: isTablet ? 30 : 26,
      fontWeight: FontWeight.w700,
      letterSpacing: -0.5,
    );
    final subStyle = TextStyle(fontSize: isTablet ? 15 : 14, color: Colors.grey.shade600);
          ...
          ListView(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  Text('Accounts Receivable', style: headerTitleStyle, maxLines: 1, overflow: TextOverflow.ellipsis),
                  Container(
              ...
              Text('Outstanding payments from customers', style: subStyle),
                ...
                children: [
                  ...
                  Expanded(
                    child: _metricCard(
                      ...
                      value: '₱${currencyFmt.format(totals.totalOverdue)}',
                      icon: Icons.warning_amber_rounded,
                      color: totals.totalOverdue > 0 ? Colors.red : Colors.green,
                    ...
                    Row(
                      children: [
                        Container(
                          ...
                          child: Icon(Icons.analytics_outlined, color: Colors.indigo.shade700, size: isTablet ? 26 : 24),
                        ),
                        const SizedBox(width: 12),
                        Text(
                          'Aging Analysis',
                          style: TextStyle(fontSize: isTablet ? 20 : 18, fontWeight: FontWeight.w800, letterSpacing: -0.3),
                        ),
                      ],
                    ),
                    const SizedBox(height: 16),
                    _agingBar(label: 'Current', amount: totals.agingTotals['current'] ?? 0, total: totals.totalAR, color: Colors.green),
                    const SizedBox(height: 10),
                    _agingBar(label: '1-30 days', amount: totals.agingTotals['1-30'] ?? 0, total: totals.totalAR, color: Colors.blue),
                    const SizedBox(height: 10),
                    _agingBar(label: '31-60 days', amount: totals.agingTotals['31-60'] ?? 0, total: totals.totalAR, color: Colors.orange),
                    const SizedBox(height: 10),
                    _agingBar(label: '61-90 days', amount: totals.agingTotals['61-90'] ?? 0, total: totals.totalAR, color: Colors.deepOrange),
                    const SizedBox(height: 10),
                    _agingBar(label: '90+ days', amount: totals.agingTotals['90+'] ?? 0, total: totals.totalAR, color: Colors.red),
                  ],
                ),
              ),
              SizedBox(height: blockGap),
              ...
              TextField(
                decoration: InputDecoration(
                  ...
                  contentPadding: EdgeInsets.symmetric(vertical: isTablet ? 16 : 14, horizontal: isTablet ? 18 : 16),
                ...
                onChanged: (v) => ref.read(arNotifierProvider.notifier).setSearch(v),
              ),
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('Client Accounts', style: TextStyle(fontSize: isTablet ? 18 : 16, fontWeight: FontWeight.w700, color: Colors.grey.shade800)),
                  Text(
                    '${clients.length} ${clients.length == 1 ? 'client' : 'clients'}',
                    style: TextStyle(fontSize: isTablet ? 15 : 14, fontWeight: FontWeight.w600, color: Colors.grey.shade500),
                  ),
                ],
              ),
              ...
              // Client Cards
              if (!isTablet)
                for (final c in clients) _arCard(context, c, isTablet: false)
              else
                _ClientGrid(
                  clients: clients,
                  buildCard: (c) => _GridClientCard(
                    client: c,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (subtitle != null) ...[
            const SizedBox(height: 4),
            Text(subtitle, style: TextStyle(fontSize: isTablet ? 12 : 11, fontWeight: FontWeight.w500, color: Colors.grey.shade500)),
          ],
        ],
      ),
    );
  ...
  Widget _arCard(BuildContext context, ClientAR c, {required bool isTablet}) {
    ...
    final statusColor = switch (status) { 'Overdue' => Colors.red, 'Partial' => Colors.orange, _ => Colors.green };
    final statusIcon = switch (status) { 'Overdue' => Icons.error_outline, 'Partial' => Icons.schedule, _ => Icons.check_circle_outline };
              ...
              children: [
                Expanded(
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(clientName,
                        ...
                        style: TextStyle(fontSize: isTablet ? 18 : 17, fontWeight: FontWeight.w700, letterSpacing: -0.3)),
                    const SizedBox(height: 4),
                ...
                Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    const SizedBox(width: 4),
                    Text(status, style: TextStyle(color: statusColor, fontWeight: FontWeight.w700, fontSize: 12, letterSpacing: 0.2)),
                  ]),
                ),
              ],
            ),
                ...
                children: [
                  Expanded(
                    child: Text(
                      ...
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                      style: TextStyle(fontSize: isTablet ? 22 : 20, fontWeight: FontWeight.w800, color: Colors.grey.shade900, letterSpacing: -0.5),
                    ),
                  ),
                  if (overdue > 0)
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text('OVERDUE',
                              style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: Colors.red.shade700, letterSpacing: 0.5)),
                          const SizedBox(height: 2),
                          Text('₱${currencyFmt.format(overdue)}',
...
class _ClientGrid extends StatelessWidget {
  final List<ClientAR> clients;
  final Widget Function(ClientAR) buildCard;

  const _ClientGrid({required this.clients, required this.buildCard});

  @override
  Widget build(BuildContext context) {
    final width = MediaQuery.sizeOf(context).width;
    // target ≈ 420px per card; clamp 2..3 columns
    ...
    return GridView.builder(
      ...
      itemBuilder: (ctx, i) => buildCard(clients[i]),
    );
  }
}
...
class _ClientDetail extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final invoices = client.invoices.where((i) => i.balance > 0).toList()
      ..sort((a, b) => (a.dueDate ?? DateTime(2100)).compareTo(b.dueDate ?? DateTime(2100)));

    final total = invoices.fold<double>(0, (sum, inv) => sum + inv.balance);
    ...
    return Container(
      ...
      child: ListView(
        ...
        shrinkWrap: true,
        children: [
          Text(client.client, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w800, letterSpacing: -0.5)),
          const SizedBox(height: 4),
          Text('Client Details', style: TextStyle(fontSize: 14, color: Colors.grey.shade600)),
          const SizedBox(height: 20),
            ...
            child: Row(
              ...
              children: [
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('Total Outstanding', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.blue.shade700)),
                  const SizedBox(height: 4),
                  Text('₱${currencyFmt.format(total)}',
                      style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: Colors.blue.shade900, letterSpacing: -0.5)),
                ]),
                Container(
          ...
          Text('Aging Breakdown', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Colors.grey.shade800)),
          const SizedBox(height: 12),
          Container(
            padding: const EdgeInsets.all(16),
            ...
            child: Column(
              children: [
                ...
                _divider(),
                _agingRow('61-90 days', client.aging['61-90'], Colors.deepOrange),
                _divider(),
          ...
          Text('Outstanding Invoices', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Colors.grey.shade800)),
          const SizedBox(height: 12),
    ...
    return Row(
      ...
      children: [
        ...
        Text('₱${currencyFmt.format(value)}',
            style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: value > 0 ? Colors.grey.shade900 : Colors.grey.shade400)),
      ],
    );
  }
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Text(inv.invoiceNo, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700)),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                  decoration: BoxDecoration(color: statusColor.withOpacity(0.1), borderRadius: BorderRadius.circular(6)),
                  child: Text(status, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: statusColor)),
              ...
              // due + amount
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Text(dueDate == null ? '—' : 'Due: ${DateFormat('MMM d, yyyy').format(dueDate)}',
                    style: TextStyle(fontSize: 13, color: Colors.grey.shade600)),
                Text('₱${currencyFmt.format(amount)}', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800, letterSpacing: -0.3)),
              ]),
            ]),
          ),
        ],
