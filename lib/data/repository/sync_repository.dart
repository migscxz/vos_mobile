...
class SyncRepository {
  ...
  Future<void> wipeLocal() async {
    ...
    for (final t in tables) {
      batch.delete(t); // assumes tables exist; valid given your schema creation
    }
  ...
  Future<void> _purgeNotIn({
    ...
    // Respect SQLite max variable limit using chunks.
    const chunk = 800;
    final keepMarkers = <String>[];
    final keepArgs = <Object?>[];
    for (var i = 0; i < ids.length; i += chunk) {
      final end = (i + chunk <= ids.length) ? i + chunk : ids.length;
      final part = ids.sublist(i, end);
      keepMarkers.add(List.filled(part.length, '?').join(','));
      keepArgs.addAll(part);
    }

    // "<pk> NOT IN (?,...) AND <pk> NOT IN (?,...) ..."
    final whereExpr = keepMarkers.map((m) => '$pk NOT IN ($m)').join(' AND ');
    await db.delete(table, where: whereExpr, whereArgs: keepArgs);
  }
  ...
  /// Normal sync. Set [purge]=true to drop local rows not present on server.
  Future<void> syncAll({bool purge = false}) async {
    ...
    await Future.wait([
      ...
      syncPostDispatchBudgeting(purge: purge),     // <-- now included
      syncPostDispatchPlanStaff(purge: purge),     // <-- now included
      ...
      // Master refs for products (ensure these first)
  ...
  Future<void> syncPostDispatchInvoices({bool purge = false}) async {
    ...
    final batch = db.batch();
    for (final r in rows) {
      batch.insert('post_dispatch_invoices', {
        'id': r['id'],
  ...
  Future<void> syncPostDispatchPlan({bool purge = false}) async {
    ...
    final batch = db.batch();
    for (final r in rows) {
      batch.insert('post_dispatch_plan', {
        'id': r['id'],
        'doc_no': r['doc_no'],
  ...
  Future<void> syncSalesInvoicePayments({bool purge = false}) async {
    ...
    final batch = db.batch();
    for (final r in rows) {
      batch.insert('sales_invoice_payments', {
        'id': r['id'],
        'invoice_id': r['invoice_id'],
        'order_id': r['order_id']?.toString(),
        'coa_id': r['coa_id'],
  ...
  Future<void> syncUnits({bool purge = false}) async {
    ...
    for (final r in rows) {
      batch.insert('units', {
        ...
        // column name is "order" in SQLite; safe to use as a map key
  ...
  Future<void> syncProductPerSupplier({bool purge = false}) async {
    ...
    final batch = db.batch();
    for (final r in rows) {
      batch.insert('product_per_supplier', {
        'id': r['id'],
        'supplier_id': r['supplier_id'],
  ...
  Future<void> syncSalesInvoiceDetails({bool purge = false}) async {
    ...
    final batch = db.batch();
    for (final r in rows) {
      batch.insert('sales_invoice_details', {
  ...
  Future<void> syncSalesReturnDetails({bool purge = false}) async {
    ...
    final batch = db.batch();
    for (final r in rows) {
      batch.insert('sales_return_details', {
        'detail_id': r['detail_id'],
        'return_no': r['return_no']?.toString(), // e.g. "SR-177"
        'product_id': r['product_id'],
        'quantity': _asDouble(r['quantity']),
        'unit_price': _asDouble(r['unit_price']),
        'total_amount': _asDouble(r['total_amount']),
  ...
  Future<void> syncSalesInvoiceSalesReturn({bool purge = false}) async {
    ...
    final batch = db.batch();
    for (final r in rows) {
      batch.insert('sales_invoice_sales_return', {
        'id': r['id'],
        'return_no': r['return_no'],       // integer return id on server
        'invoice_no': r['invoice_no'],
  ...
  Future<void> syncDivisions({bool purge = false}) async {
    final db = await _db;
    final rows = await api.getList('/items/divisions?limit=-1');
    final batch = db.batch();
    for (final r in rows) {
    ...
    if (purge) {
      await _purgeNotIn(
        table: 'divisions',
        pk: 'division_id',
        remoteIds: rows.map((e) => e['division_id']),
      );
    }
  }
  ...
  Future<void> syncSalesInvoiceType({bool purge = false}) async {
    ...
    final batch = db.batch();
    for (final r in rows) {
      batch.insert('sales_invoice_type', {
        'id': r['id'],
        'type': r['type'],
        'shortcut': r['shortcut'],
  ...
  Future<void> syncCustomers({bool purge = false}) async {
    ...
    for (final r in rows) {
      batch.insert('customer', {
        ...
        'brgy': r['brgy'],           // needed for address concat
        'city': r['city'],
        'province': r['province'],   // needed for address concat
      }, conflictAlgorithm: ConflictAlgorithm.replace);
    }
    await batch.commit(noResult: true);
  ...
  Future<void> syncPostDispatchBudgeting({bool purge = false}) async {
    ...
    final batch = db.batch();
    for (final r in rows) {
      batch.insert('post_dispatch_budgeting', {
        'id': r['id'],
        'post_dispatch_plan_id': r['post_dispatch_plan_id'],
        'coa_id': r['coa_id'],
        'remarks': r['remarks'],
  ...
  Future<void> syncPostDispatchPlanStaff({bool purge = false}) async {
    ...
    final batch = db.batch();
    for (final r in rows) {
      batch.insert('post_dispatch_plan_staff', {
        'id': r['id'],
        'post_dispatch_plan_id': r['post_dispatch_plan_id'],
        'user_id': r['user_id'],
        'role': r['role'],
  ...
  Future<void> syncDisbursementPayables({bool purge = false}) async {
    ...
    final batch = db.batch();
    for (final r in rows) {
      batch.insert('disbursement_payables', {
        'id': r['id'],
  ...
  Future<void> syncDisbursementPayments({bool purge = false}) async {
    ...
    final batch = db.batch();
    for (final r in rows) {
      batch.insert('disbursement_payments', {
        'id': r['id'],
        'disbursement_id': r['disbursement_id'],
        'coa_id': r['coa_id'],
        'bank_id': r['bank_id'],
  ...
  Future<List<Map<String, Object?>>> getDeliveryReportFiltered({
    ...
    if (search != null && search.trim().isNotEmpty) {
      where.add("(customer_name LIKE ? OR invoice_no LIKE ? OR doc_no LIKE ? OR city_town_name LIKE ?)");
